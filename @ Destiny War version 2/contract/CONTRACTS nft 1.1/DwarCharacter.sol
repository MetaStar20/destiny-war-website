...
contract DestinyWarCharacter is ERC721Enumerable, Ownable {
  ...
  function _mintSingleNFT(address to) private {
    ...
    bool isRare;
    if (newTokenId < 336) isRare = true;
     else isRare = newTokenId % rarePerNormal == 0;
    ...
    characters[newTokenId] = Character({ tokenId: newTokenId, tokenURI: realTokenURI, isRare: isRare, isMale: isMale });
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    bool isRare = tokenId > 335 && tokenId % rarePerNormal == 0;
    string memory currentBaseURI = isRare ? rareTokenURI : normalTokenURI;
  ...
  /* GETTER */
  function getCharacter(uint256 _tokenId) external view returns (uint256, string memory, bool, bool) {
    return (characters[_tokenId].tokenId, characters[_tokenId].tokenURI, characters[_tokenId].isRare, characters[_tokenId].isMale);
  }
