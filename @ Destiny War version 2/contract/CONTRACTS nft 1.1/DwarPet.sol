...
contract DestinyWarPet is ERC721Enumerable, Ownable {
  ...
  function _mintSingleNFT(address to) private {
    ...
    pets[newTokenId] = Pet({ tokenId: newTokenId, tokenURI: realTokenURI, isMale: isMale });
  ...
  /* GETTER */
  function getPet(uint256 _tokenId) external view returns (uint256, string memory, bool) {
    return (pets[_tokenId].tokenId, pets[_tokenId].tokenURI, pets[_tokenId].isMale);
  }
